// TODO: rewrite to use kysely/prisma
...
export default function vercelPostgresAdapter(): Adapter {
  try {
    const createUser = async (
      ...
      if (!rows || !rows[0]) throw new Error("No rows returned from database");
    ...
    const createVerificationToken = async ({
      ...
      const expires = new Date(rawExpires).toISOString();
